...
This presentation isn't just about computer chips; it's a direct application of the \textbf{Physical Electronics} curriculum. Use these connections to ground your talk in what the class has already learned.

\subsection*{1. Semiconductor Physics $\to$ The MOSFET (Slide 2)}
\begin{itemize}
    \item \textbf{Class Topic:} Energy Bands \& Carrier Transport.
...
\subsection*{2. Quantitative Analysis $\to$ Square Law (Slide 3)}
\begin{itemize}
...
\subsection*{3. Device Modeling $\to$ Dynamic Power (Slide 7)}
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{The Link:} ARM's efficiency isn't magic; it's a result of optimizing these physical parameters. By simplifying the architecture (RISC), they reduce the number of physical devices that need to switch, effectively lowering the total capacitance ($C_{load}$) the system must charge.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Integration:} You can fit the CPU + GPU + NPU + Memory Controller all on one small chip (SoC).
    \item \textbf{Synergy:} putting them close together shortens the wires between them. Short wires = less capacitance = even lower power.
\end{itemize}
\textbf{Conclusion:} ARM isn't just about writing code differently. It's a strategy to minimize the number of electrons you have to move to get a job done.

\newpage

\section{Glossary + how to say symbols out loud}
...
\section{Slide-by-slide: what each slide means + what to say}
This section is the core rehearsal material.

\Slide{1 --- Title: The ARM Advantage}
\Goal{Set the thesis and the direction: physics \(\to\) architecture \(\to\) efficiency \(\to\) SoC.}
\PointAt{The 4-step flow (MOSFET \(\to\) CMOS \(\to\) RISC \(\to\) SoC) and the thesis box.}
\Say\\
...
\PointAt{OFF vs ON visual: gate label and channel line.}
\Say\\
Every CPU starts with one physical device: the MOSFET. It\textquotesingle s basically a voltage-controlled switch. When the gate voltage is low, there\textquotesingle s no channel, so current can\textquotesingle t flow. When the gate voltage is high enough, it creates an electric field that forms a channel, and current flows from source to drain.

\IfAsked\\
A helpful analogy is a water valve: the gate voltage opens or closes the valve, controlling flow. In reality it\textquotesingle s an electric field shaping the energy barrier in silicon, but for this talk, \emph{``voltage opens a path for current''} is the right mental model.
...
\PointAt{The current proportionality and the two knobs (Geometry / Overdrive).}
\Say\\
...
\Say\\
We build logic using CMOS: PMOS pulls the output up to 1, NMOS pulls it down to 0. A NAND gate uses four transistors and is universal: you can build any digital circuit from NAND gates. So when we talk about CPU architecture, it\textquotesingle s really a massive organization of these gate-level switching events.
...
\Say\\
Dynamic power is approximately \(P_{dynamic} \approx C_{load}V_{DD}^2 f\). This is why voltage matters so much: it\textquotesingle s squared. ARM benefits by reducing \(C_{load}\) through simpler overhead, and by targeting designs that run well at lower voltage. Even a modest voltage drop can massively reduce power.
...
\Transition{There\textquotesingle s always a trade-off: lowering voltage also slows switching.}
...
\PointAt{Compute / Memory / I/O cards and the SoC block map.}
\Say\\
A real product is a system: CPU, GPU, NPU, memory controllers, and physical I/O. Integration shortens wires, which reduces capacitance and energy per operation. ARM is not just a core; it\textquotesingle s an ecosystem and design approach that fits well into SoCs.
...
\Say\\
The physics takeaway is that switching and voltage dominate energy. The architecture takeaway is that RISC reduces overhead switching, lowering \(C_{load}\). The result is lower power and easier integration --- ARM isn\textquotesingle t just code; it\textquotesingle s a strategy for organizing silicon.
...
\]
We dropped \(\alpha\) on slides to keep it simple, but it\textquotesingle s there in real chip modeling.

\subsection{Why decode/control can dominate}
Control logic can switch a lot even when the instruction does little. Irregular instruction formats also increase wiring complexity. The point isn\textquotesingle t that \emph{all} x86 chips are inefficient; it\textquotesingle s that the architecture historically pushed more work into front-end decode.

\subsection{Common nuance: ISA vs microarchitecture}
...
\begin{itemize}
  ...
  A: It\textquotesingle s the amount of ``electrical stuff you have to charge'' each time logic switches --- transistors and wires.

  \item \textbf{Q: What\textquotesingle s a SoC?}\\
...
\begin{enumerate}
  ...
  \item More current \(\Rightarrow\) faster switching (but costs area/voltage).
  \item CMOS gates (like NAND) build all logic.
  \item RISC reduces decode overhead \(\Rightarrow\) lower \(C_{load}\).
  \item Dynamic power: \(P \approx C V^2 f\) (voltage is squared).
\end{enumerate}

\textbf{Three transitions to remember:}
\begin{itemize}
  \item MOSFET \(\to\) current sets speed.
  \item Gates \(\to\) architecture changes how much switches.
  \item Lower \(C\) and \(V\) \(\to\) better perf/W \(\to\) SoC integration.
\end{itemize}
